...
// ================== STORAGE HELPERS (events & updates only) ==================
function getEvents() {
...
// ================== BACKEND HELPERS (board) ==================
async function apiGetBoard() {
  const res = await fetch(`${API_BASE}/api/board`);
  return res.json();
}

async function apiAddBoard(member) {
  const res = await fetch(`${API_BASE}/api/board`, {
    method: "POST",
...
async function apiUpdateBoard(id, member) {
  const res = await fetch(`${API_BASE}/api/board/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(member),
  });
...
async function apiDeleteBoard(id) {
  const res = await fetch(`${API_BASE}/api/board/${id}`, {
    method: "DELETE",
  });
  return res.json();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (boardSaveBtn && boardResetBtn) {
    ...
    boardSaveBtn.onclick = async () => {
      ...
      const editIndex = document.getElementById("boardEditIndex").value;
      ...
      try {
        if (editIndex && editIndex !== "-1") {
          // update existing
          const id = Number(editIndex);
          await apiUpdateBoard(id, { name, role, year, img: imgData });
          msg.textContent = "Board member updated!";
        } else {
          // add new
          await apiAddBoard({ name, role, year, img: imgData });
        ...
        console.error(err);
        msg.textContent = "Error saving to backend.";
        msg.style.color = "red";
      }
    };
  }
...
async function renderAdminBoard() {
  ...
  let list = [];
  try {
    list = await apiGetBoard();
  } catch (err) {
    console.error(err);
    container.innerHTML = "<p>Error loading from backend.</p>";
    return;
  }
  ...
  sorted.forEach((m) => {
    ...
    row.innerHTML = `
      ...
      <div class="admin-event-actions">
        <button class="editBtnBoard" data-id="${m.id}">Edit</button>
        <button class="deleteBtnBoard" data-id="${m.id}">Delete</button>
      </div>
    `;
    container.appendChild(row);
  });
  ...
  container.querySelectorAll(".editBtnBoard").forEach((btn) => {
    btn.onclick = async () => {
      ...
      document.getElementById("boardEditIndex").value = String(id);
      const msg = document.getElementById("boardMsg");
      if (msg) {
...
// ================== PUBLIC: BOARD PAGE ==================
async function renderBoardPublic() {
  ...
  try {
    ...
    console.error(err);
    container.innerHTML = "<p>Error loading board from backend.</p>";
    return;
  }
  ...
  sorted.forEach((m) => {
    ...
    card.innerHTML = `
      ...
      ${m.year ? `<p class="board-year">${m.year}</p>` : ""}
    `;
    container.appendChild(card);
  });
}
